...
class ImageContainer:
    ...
    def load(self):
        ...
        if self.imageType == InputType.imageGrayscale:
            ...
            self.state = LoadState.loaded
        elif self.imageType == InputType.fixationMap:
            self.data = cv2.cvtColor(cv2.imread(self.filePath, cv2.IMREAD_COLOR), cv2.COLOR_BGR2GRAY)
            #self.data = np.loadtxt(self.filePath, dtype='i')
            self.state = LoadState.loaded
    ...
    def getImage(self):
        ...
        if self.imageType == InputType.image:
            if self.state == LoadState.unloaded:
                return cv2.cvtColor(cv2.resize(cv2.imread(self.filePath, cv2.IMREAD_COLOR), (256,192)), cv2.COLOR_BGR2RGB)
            elif self.state == LoadState.loaded:
                return self.data
            elif self.state == LoadState.loadedCompressed:
...
class Target():
    def __init__(self, imagePath, saliencyPath,fixationPath, weimapPath,
                 ...
                 weimapState=LoadState.unloaded, weimapType=InputType.imageGrayscale):
